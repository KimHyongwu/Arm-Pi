...
from ros_robot_controller.ros_robot_controller_sdk import Board, StepperSubdivision as sdkStepperSub
from ros_robot_controller_msgs.srv import GetBusServoState, GetPWMServoState,MoveStepper
...
class RosRobotController(Node):
    ...
    def __init__(self, name):
        ...
        self.create_subscription(ConveyorSpeed, '~/set_conveyor_speed', self.set_conveyor_speed_callback, 10)
        self.create_subscription(StepperSubdivision, '~/set_stepper_subdivision', self.set_stepper_subdivision_callback, 10)
    ...
    def get_bus_servo_state(self, request, response):
        ...
        return response
    def set_conveyor_speed_callback(self, msg):
        #self.get_logger().info(f"传送带速度为: {msg.speed}")
        self.board.set_conveyor_speed(msg.speed)

    def set_stepper_subdivision_callback(self, msg):
        try:
            subdiv_enum = sdkStepperSub(msg.subdivision)
            #self.get_logger().info(f"步进电机细分为: {subdiv_enum.name} ({hex(msg.subdivision)})")
            self.board.set_stepper_motor_subdivision(subdiv_enum)
        except ValueError:
            #self.get_logger().error(f"无效的细分值: {msg.subdivision}。请使用有效的枚举值。")
            pass

    def move_stepper_callback(self, request, response):
        #self.get_logger().info(f"收到步进电机移动请求: {request.steps} 步。开始执行...")
        success = self.board.set_stepper_motor_position(request.steps)
        
        if success:
            response.success = True
            response.message = "Stepper motor movement command successfully executed or initialized successfully"
            self.get_logger().info(response.message)
        else:
            response.success = False
            response.message = "Stepper motor movement or initialization failure。"
